...
function housingAfterLoan() {
	layui.use([ 'table' ], function() {
		...
		table.render({
			...
			url : '/PostLoanManage/findAllHouseProperty.action',
			page : {
				theme : '#405467'
			},
		...
		var active = {
			reload : function() {
				...
				table.reload('testReload', {
					url : '/PostLoanManage/findHousePropertyByPhone.action',
					page : {
						curr : 1
					// 重新从第 1 页开始
					},
		...
		table.on('tool(realEstateMortgage)', function(obj) {
			...
			// console.log(data);
			if (obj.event == 'see') {
				window.location.href = "housingAfterLoanInfo.jsp?entry_number=" + data.entry_number;
			}
		});
	})
			...
			$.ajax({
				...
				success : function(result) {
					...
					layer.alert('提交成功', {
						...
						window.location.href = 'housingAfterLoan.jsp'
					});
