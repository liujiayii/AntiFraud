...
$(document).ready(function() {
	$($('#supplementaryFormalities dd')[0]).addClass('layui-this');
	$($('.layui-side .layui-nav-item')[3]).addClass('layui-nav-itemed');
});

function housingFormalities() {
	layui.use([ 'table' ], function() {
		...
		table.render({
			...
			url : '/ReplenishProcedureImage/findAllHouseProperty.action',
			page : {
				theme : '#405467'
			},
			even : true,
		...
		var active = {
			reload : function() {
				...
				table.reload('testReload', {
					url : '/ReplenishProcedureImage/findHousePropertyByPhone.action',
					page : {
						curr : 1
					// 重新从第 1 页开始
					},
		...
		table.on('tool(realEstateMortgage)', function(obj) {
			...
			console.log(obj);
			if (obj.event == 'see') {
				window.location.href = "housingFormalitiesInfo.jsp?id=" + data.id;
			}
		});
...
function housingFormalitiesInfo() {
	onLoadPage('id');
			...
			$.ajax({
				...
				success : function(result) {
					...
					layer.alert('修改成功', {
						...
						window.location.href = "housingFormalities.jsp"
					});
				}
			});
...
function onLoadPage(name) {
	...
	$.ajax({
		url : '/ReplenishProcedure/showInfoByHouseId.action',
		type : 'post',
		dataType : 'json',
		data : {
			id : id
