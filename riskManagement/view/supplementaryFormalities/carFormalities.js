...
$(document).ready(function() {
	$($('#supplementaryFormalities dd')[1]).addClass('layui-this');
	$($('.layui-side .layui-nav-item')[3]).addClass('layui-nav-itemed');
});

function carFormalities() {
	layui.use([ 'table' ], function() {
		...
		table.render({
			...
			url : '/ReplenishProcedureImage/findAllVehicle.action',
			page : {
				theme : '#405467'
			},
			even : true,
		...
		var active = {
			reload : function() {
				...
				table.reload('testReload', {
					url : '/ReplenishProcedureImage/findVehicleByPhone.action',
					page : {
						curr : 1
					// 重新从第 1 页开始
					},
			...
			$.ajax({
				...
				success : function(result) {
					...
					layer.alert('修改成功', {
						...
						window.location.href = "carFormalities.jsp"
					});
				}
			});
