...
$(document).ready(function() {
	$($('#managementAfterLoan dd')[1]).addClass('layui-this');
	$($('.layui-side .layui-nav-item')[7]).addClass('layui-nav-itemed');
});

function carAfterLoan() {
	layui.use([ 'table' ], function() {
		...
		table.render({
			...
			url : '/PostLoanManage/findAllVehicle.action',
			page : {
				theme : '#405467'
			},
		...
		var active = {
			reload : function() {
				...
				table.reload('testReload', {
					url : '/PostLoanManage/findVehicleByPhone.action',
					page : {
						curr : 1
					// 重新从第 1 页开始
					},
		...
		table.on('tool(realEstateMortgage)', function(obj) {
			...
			// console.log(data);
			if (obj.event == 'see') {
				window.location.href = "carAfterLoanInfo.jsp?entry_number=" + data.entry_number;
			}
		});
	})
			...
			$.ajax({
				...
				success : function(result) {
					...
					layer.alert('提交成功', {
						...
						window.location.href = 'carAfterLoan.jsp'
					});
				}
			})
