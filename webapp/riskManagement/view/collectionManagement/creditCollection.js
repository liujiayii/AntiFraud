...
$(document).ready(function() {
	$($('#collectionManagement dd')[2]).addClass('layui-this');
...
function creditCollection() {
	layui.use([ 'table' ], function() {
		...
		table.render({
			...
			url : '/LiquidateManage/findAllFiduciary.action',
			page : {
				theme : '#405467'
			},
			id : 'testReload',
		...
		var active = {
			reload : function() {
				...
				table.reload('testReload', {
					url : '/LiquidateManage/findFiduciaryByPhone.action',
					page : {
						curr : 1
					// 重新从第 1 页开始
					},
		...
		table.on('tool(realEstateMortgage)', function(obj) {
			...
			console.log(obj);
			if (obj.event == 'see') {
				window.location.href = "creditCollectionInfo.jsp?entry_number=" + data.entry_number;
			}
		});
...
function onLoadPage(name) {
	...
	$.ajax({
		url : '/LiquidateManage/findFiduciaryByEntry_number.action',
		type : 'post',
		dataType : 'json',
		data : {
			id : entry_number
