...
public class LoginController {
	...
	public ModelAndView toLogin(HttpServletRequest request) {	
		...
		//session.setAttribute("isIn", isIn);
		
		System.out.println("U盾状态："+ISIN);
		/** 生成随机数 */
	...
	public Map<String, Object> loginUser(String username, String password, HttpSession session, HttpServletRequest req,
		...
		System.out.println("地址："+System.getProperty("java.library.path"));
		int code = 0;
		int img = 0;
		String msg = null;
		Map<String, Object> map = new HashMap<>();
		...
		if (!StringUtils.isEmpty(KeyID)) {
			...
			try {
				...
				System.out.println("user.getImg_url()"+user.getImg_url());
				if (user.getImg_url()==null ||user.getImg_url().equals("") ) {
					// 查询数据库没有图片
					img = 0;
					map.put("img", img);
				...
				System.out.println("Session -----> setUser_id"+user.getId());
			} catch (AuthenticationException ex) {
				System.out.println("登陆失败: " + ex.getMessage());
				code = -1;
				msg = "登录失败";
